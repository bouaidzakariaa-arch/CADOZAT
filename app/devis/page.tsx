  ...
  return (
    <div ref={ref} style={{ position:'relative', marginTop:14 }}>
      <button type="button" onClick={() => setOpen(!open)} style={{
        ...
        boxShadow: open ? '0 0 0 3px rgba(204,0,0,0.08), 0 8px 24px rgba(0,0,0,0.08)' : '0 2px 12px rgba(0,0,0,0.06)',
        transition:'all .2s',
      }}>
        {selected && selectedSerie ? (
          <span style={{ width:8, height:8, borderRadius:'50%', background:'#22c55e', flexShrink:0 }}/>
        ) : (
          <span style={{ width:8, height:8, borderRadius:'50%', background:'#e5e2dd', flexShrink:0 }}/>
        )}
        <span style={{ flex:1, color: selected ? '#111' : '#bbb', fontWeight: selected ? 600 : 400 }}>
          {selected ? selected.nom : 'Choisissez un véhicule'}
        </span>
        {selected && selectedSerie && (
          <span style={{ fontSize:10, fontWeight:800, padding:'2px 8px', borderRadius:99, color:'#fff', background: selectedSerie.couleur }}>
            {selectedSerie.serie}
          </span>
        )}
        <svg style={{ position:'absolute', right:14, top:'50%', transform:`translateY(-50%) rotate(${open?180:0}deg)`, transition:'transform .2s' }}
          ...
          {vehiculesData.map((serie, si) => (
            <div key={si}>
              <div style={{
                ...
                background: serie.couleur + '08',
                borderBottom:`1px solid ${serie.couleur}18`,
                borderTop: si > 0 ? `1px solid rgba(0,0,0,0.05)` : 'none',
                display:'flex', alignItems:'center', gap:8,
              }}>
                <div style={{ width:3, height:14, borderRadius:99, background:serie.couleur }}/>
                <span style={{ fontSize:10, fontWeight:900, color:serie.couleur, textTransform:'uppercase', letterSpacing:'1.5px' }}>
                  {serie.serie}
                </span>
              </div>
              {serie.modeles.map((m, mi) => (
                <button key={mi} type="button"
                  ...
                  style={{
                    width:'100%', padding:'11px 16px 11px 28px',
                    background: value === m.value ? serie.couleur + '10' : 'transparent',
                    border:'none', cursor:'pointer', textAlign:'left', fontFamily:'inherit',
                    fontSize:13, color: value === m.value ? serie.couleur : '#444',
                    fontWeight: value === m.value ? 700 : 400,
                    display:'flex', alignItems:'center', gap:10,
                    borderBottom: mi < serie.modeles.length - 1 ? '1px solid rgba(0,0,0,0.04)' : 'none',
                ...
                >
                  {value === m.value ? (
                    <svg width="14" height="14" fill="none" stroke={serie.couleur} strokeWidth="3" viewBox="0 0 24 24" style={{ flexShrink:0 }}>
...
function Badge() {
  ...
  if (!v) return null
  return (
    <div style={{ display:'inline-flex', alignItems:'center', gap:8, marginTop:18, background:'rgba(255,255,255,0.85)', backdropFilter:'blur(12px)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:99, padding:'8px 18px', fontSize:13, fontWeight:600, color:'#444', boxShadow:'0 2px 16px rgba(0,0,0,0.06)' }}>
      <span style={{ width:8, height:8, borderRadius:'50%', background:'#22c55e', flexShrink:0 }}/>
...
function DevisForm() {
  ...
  const onFocus = (e: React.FocusEvent<HTMLInputElement|HTMLTextAreaElement>) => {
    ...
    e.target.style.boxShadow = '0 0 0 3px rgba(204,0,0,0.07)'
  }
  ...
  return (
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 24px 72px' }}>
      ...
      <div style={{
        ...
        borderRadius:24, border:'1px solid rgba(255,255,255,0.90)',
        boxShadow:'0 20px 64px rgba(0,0,0,0.08), 0 1px 0 rgba(255,255,255,0.8) inset',
        overflow:'hidden',
      }}>
        ...
        {success ? (
          <div style={{ padding:'72px 32px', textAlign:'center' }}>
            ...
            <h2 style={{ fontSize:24, fontWeight:900, color:'#111', marginBottom:8 }}>Demande envoyée !</h2>
          ...
          <form onSubmit={onSubmit}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr' }}>
              ...
              {/* ── Colonne gauche ── */}
              <div style={{ padding:38, borderRight:'1px solid rgba(0,0,0,0.06)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:26 }}>
                  <div style={{ width:36, height:36, borderRadius:'50%', background:'#CC0000', color:'#fff', fontWeight:900, fontSize:15, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow:'0 4px 14px rgba(204,0,0,0.28)' }}>1</div>
                  <div style={{ fontSize:17, fontWeight:700, color:'#111' }}>{isPre ? 'Votre véhicule' : 'Sélectionnez votre véhicule'}</div>
                </div>

                {isPre ? (
                  <div style={{ borderRadius:16, border:'1px solid rgba(0,0,0,0.08)', overflow:'hidden', background:'rgba(255,255,255,0.85)', backdropFilter:'blur(8px)' }}>
                    <div style={{ height:230, background:'rgba(255,255,255,0.9)', display:'flex', alignItems:'center', justifyContent:'center', position:'relative' }}>
                      {imgSrc
                        ? <Image src={imgSrc} alt={selV?.nom||''} fill sizes="(max-width:768px) 100vw, 50vw" style={{ objectFit:'contain', padding:20 }}/>
                        : <div style={{ textAlign:'center', color:'#ddd' }}><CarIcon/></div>
                      }
                    </div>
                    <div style={{ padding:'14px 18px', borderTop:'1px solid rgba(0,0,0,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between', background:'rgba(248,247,246,0.9)' }}>
                      <div>
                        <div style={{ fontSize:13, fontWeight:700, color:'#111' }}>{selV?.nom}</div>
                        {selS && <span style={{ display:'inline-block', marginTop:5, padding:'2px 10px', borderRadius:99, fontSize:11, fontWeight:700, color:'#fff', background:selS.couleur }}>{selS.serie}</span>}
                      </div>
                  ...
                  <>
                    {fd.vehicule ? (
                      <div style={{ minHeight:200, borderRadius:14, border:'1px solid rgba(0,0,0,0.08)', background:'rgba(255,255,255,0.85)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
                        <div style={{ width:'100%', textAlign:'center', padding:18 }}>
                          ...
                          {selS && <span style={{ display:'inline-block', marginTop:6, padding:'2px 10px', borderRadius:99, fontSize:11, fontWeight:700, color:'#fff', background:selS.couleur }}>{selS.serie}</span>}
                        </div>
                      </div>
                    ) : (
                      <div style={{ minHeight:200, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', borderRadius:14, border:'1.5px dashed rgba(0,0,0,0.10)', color:'#ccc', fontSize:13, gap:10, textAlign:'center', background:'rgba(255,255,255,0.5)' }}>
                        <CarIcon/>
              ...
              <div style={{ padding:38 }}>
                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:26 }}>
                  <div style={{ width:36, height:36, borderRadius:'50%', background:'#CC0000', color:'#fff', fontWeight:900, fontSize:15, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow:'0 4px 14px rgba(204,0,0,0.28)' }}>2</div>
                  <div style={{ fontSize:17, fontWeight:700, color:'#111' }}>Vos coordonnées</div>
                </div>

                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
                  <div>
                    <label style={lb}>Nom *</label>
                    <input name="nom" value={fd.nom} onChange={onChange} onFocus={onFocus} onBlur={onBlur} required placeholder="Votre nom" style={inp}/>
                  </div>
                  <div>
                    <label style={lb}>Prénom *</label>
                    <input name="prenom" value={fd.prenom} onChange={onChange} onFocus={onFocus} onBlur={onBlur} required placeholder="Votre prénom" style={inp}/>
                  </div>
                </div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
                  <div>
                    ...
                    <input type="tel" name="telephone" value={fd.telephone} onChange={onChange} onFocus={onFocus} onBlur={onBlur} required placeholder="06 00 00 00 00" style={inp}/>
                  </div>
                  <div>
                    <label style={lb}>Email *</label>
                    <input type="email" name="email" value={fd.email} onChange={onChange} onFocus={onFocus} onBlur={onBlur} required placeholder="votre@email.com" style={inp}/>
                  </div>
                </div>
                ...
                <div style={{ marginBottom:14 }}>
                  ...
                  <textarea name="message" value={fd.message} onChange={onChange} onFocus={onFocus} onBlur={onBlur} placeholder="Précisez vos besoins, options souhaitées, délais..."
                ...
                <button type="submit" disabled={loading} style={{
                  width:'100%', padding:14, background:'#CC0000', color:'#fff', border:'none',
                  borderRadius:12, fontSize:14, fontWeight:700, cursor:loading?'not-allowed':'pointer',
                  display:'flex', alignItems:'center', justifyContent:'center', gap:8,
                  fontFamily:'inherit', opacity:loading?0.7:1,
                  boxShadow:'0 6px 22px rgba(204,0,0,0.28)', transition:'all .2s',
                }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14, marginTop:22 }}>
        {[
          ...
          <div key={i} style={{ background:'rgba(255,255,255,0.80)', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,0.90)', borderRadius:16, padding:'16px 18px', display:'flex', alignItems:'center', gap:12, boxShadow:'0 4px 18px rgba(0,0,0,0.05)' }}>
            <div style={{ width:42, height:42, borderRadius:'50%', background:c.bg, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{c.icon}</div>
            <div>
              <div style={{ fontSize:11, color:'#bbb', marginBottom:3 }}>{c.lbl}</div>
              <div style={{ fontSize:13, fontWeight:700, color:'#111' }}>{c.val}</div>
            </div>
          </div>
        ))}
      </div>
    ...
    <div style={{ position:'relative', minHeight:'100vh', overflowX:'hidden' }}>
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        <div style={{ maxWidth:1100, margin:'0 auto', padding:'52px 24px 36px' }}>
          ...
          <h1 style={{ fontSize:48, fontWeight:900, color:'#111', letterSpacing:'-1.5px', lineHeight:1, marginBottom:14 }}>

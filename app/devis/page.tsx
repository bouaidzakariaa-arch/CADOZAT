...
function DevisForm() {
  ...
  const vehiculeParam = searchParams.get('vehicule') // ex: "dmax-tfr-sc-4x2-clim"

  // Vérifie si le param URL correspond à un vrai modèle
  const allModeles   = vehiculesData.flatMap(s => s.modeles)
  const validParam   = vehiculeParam && allModeles.some(m => m.value === vehiculeParam)
    ? vehiculeParam : null

  // Si on arrive depuis une page véhicule → véhicule fixe, pas de select
            ...
            <div className="grid lg:grid-cols-2 gap-12">
              ...
              <div>
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl font-black text-[#1B2B6B]">
                    {isPreselected ? 'Votre véhicule' : 'Sélectionnez votre véhicule'}
                  </h2>
                </div>

                {/* ── CAS 1 : véhicule pré-sélectionné → image grande + nom, pas de select ── */}
                {isPreselected ? (
                  <div className="rounded-2xl bg-gray-50 border-2 border-gray-100 overflow-hidden">
                    ...
                    <div className="relative w-full h-64 bg-white">
                      {imageSrc ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center text-gray-300">
                          ...
                          <p className="text-sm">Image bientôt disponible</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
                      <div>
                        <p className="font-bold text-gray-800">{selectedVehicle?.nom}</p>
                        {selectedSerie && (
                          <span className="inline-block mt-1 px-3 py-0.5 rounded-full text-xs font-bold text-white" style={{ background: selectedSerie.couleur }}>
                            {selectedSerie.serie}
                          </span>
                        )}
                      </div>
                      ...
                      <Link
                        href="/devis"
                        className="text-xs text-gray-400 hover:text-[#CC0000] underline underline-offset-2 transition-colors"
                      >
                        Changer
                      </Link>
                    </div>
                  ...
                  /* ── CAS 2 : pas de param → select + aperçu image ── */
                  <>
                    <div className={`rounded-2xl mb-6 min-h-[220px] flex items-center justify-center relative overflow-hidden transition-all duration-300 ${
                      ...
                      {formData.vehicule ? (
                        <div className="w-full text-center px-6 py-6">
                          <div className="relative w-full h-40 mx-auto mb-3">
                            {imageSrc ? (
                              <Image key={formData.vehicule} src={imageSrc} alt={selectedVehicle?.nom || ''} fill className="object-contain drop-shadow-md"/>
                            ) : (
                              <div className="w-full h-full flex flex-col items-center justify-center text-gray-300">

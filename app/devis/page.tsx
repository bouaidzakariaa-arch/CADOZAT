...
// Données des véhicules (même structure que le menu)
const vehiculesData = [
  {
    serie: 'D-MAX',
    couleur: '#CC0000',
...
export default function DevisPage() {
  ...
  // Trouver le véhicule sélectionné pour afficher l'image
  const selectedVehicle = vehiculesData
    .flatMap(s => s.modeles)
    .find(m => m.value === formData.vehicule)

  const selectedSerie = vehiculesData.find(s => 
    s.modeles.some(m => m.value === formData.vehicule)
  )
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
          ...
          {success ? (
            <div className="p-12 text-center">
              ...
              <Link 
                href="/"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#CC0000] text-white font-bold rounded-full hover:bg-[#aa0000] transition-colors"
              >
                Retour à l'accueil
            ...
            <form onSubmit={handleSubmit} className="p-8 md:p-12">
              <div className="grid lg:grid-cols-2 gap-12">
                ...
                <div>
                  ...
                  {/* Image du véhicule */}
                  <div className="bg-gray-50 rounded-2xl p-8 mb-6 min-h-[250px] flex items-center justify-center">
                    {formData.vehicule ? (
                      <div className="text-center">
                        <div className="relative w-64 h-40 mx-auto mb-4">
                          <Image 
                            src={`/images/vehicules/${formData.vehicule}.png`}
                            alt={selectedVehicle?.nom || 'Véhicule'}
                            fill
                            className="object-contain"
                            onError={(e) => {
                              (e.target as HTMLImageElement).src = '/images/vehicule-placeholder.png'
                            }}
                          />
                        </div>
                        <p className="font-bold text-[#1B2B6B]">{selectedVehicle?.nom}</p>
                        {selectedSerie && (
                          <span 
                            className="inline-block mt-2 px-3 py-1 rounded-full text-xs font-bold text-white"
                            style={{ background: selectedSerie.couleur }}
                          >
                            {selectedSerie.serie}
                          </span>
                        )}
                      ...
                      <div className="text-center text-gray-400">
                        <svg className="w-24 h-24 mx-auto mb-4 opacity-30" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  <p className="text-xs text-gray-400 mt-4">
                    * Photos non contractuelles. Des différences en termes de spécifications, motorisations, options et couleurs disponibles peuvent exister.
                  </p>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={handleChange}
                        rows={4}
                        placeholder="Précisez vos besoins, options souhaitées, délais..."
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input
                        ...
                        onChange={handleChange}
                        className="mt-1 w-5 h-5 text-[#CC0000] border-2 border-gray-300 rounded focus:ring-[#CC0000]"
                      />
                      <span className="text-sm text-gray-600">
                        J'accepte de recevoir les offres commerciales et actualités de CADOZAT par email.
                      </span>
                    </label>
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full py-4 bg-[#CC0000] text-white font-bold text-lg rounded-xl hover:bg-[#aa0000] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg hover:shadow-xl"
                    >
                      {loading ? (
                        <>
                    ...
                    <p className="text-xs text-gray-400 text-center">
                      En soumettant ce formulaire, vous acceptez notre <Link href="/mentions-legales" className="text-[#CC0000] hover:underline">politique de confidentialité</Link>.
                    </p>
                  </div>
                </div>
              </div>

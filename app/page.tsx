...
function AnimatedBg({ couleur }: { couleur: string }) {
  ...
  useEffect(() => {
    ...
    const r = parseInt(couleur.slice(1,3),16), g = parseInt(couleur.slice(3,5),16), b = parseInt(couleur.slice(5,7),16)
    const pts = Array.from({ length: 55 }, () => ({ x: Math.random()*W, y: Math.random()*H, r: Math.random()*2.2+0.8, vx:(Math.random()-0.5)*0.4, vy:(Math.random()-0.5)*0.4, op:Math.random()*0.35+0.1 }))
    const draw = () => {
      ...
      for(let i=0;i<pts.length;i++) for(let j=i+1;j<pts.length;j++) {
        ...
        if(dist<120) { ctx.beginPath(); ctx.moveTo(pts[i].x,pts[i].y); ctx.lineTo(pts[j].x,pts[j].y); ctx.strokeStyle=`rgba(${r},${g},${b},${0.06*(1-dist/120)})`; ctx.lineWidth=0.5; ctx.stroke() }
      }
...
function ModelCard({ m, couleur, index }: { m: any; couleur: string; index: number }) {
  ...
  return (
    <Link ref={ref} href={m.href} className="group block bg-white rounded-2xl overflow-hidden"
      style={{ border:'1px solid rgba(0,0,0,0.07)', boxShadow:'0 2px 16px rgba(0,0,0,0.06)', opacity:visible?1:0, transform:visible?'translateY(0) scale(1)':'translateY(32px) scale(0.97)', transition:`opacity 0.5s cubic-bezier(0.22,1,0.36,1) ${index*80}ms, transform 0.5s cubic-bezier(0.22,1,0.36,1) ${index*80}ms, box-shadow 0.2s, border-color 0.2s` }}
      ...
      <div className="relative overflow-hidden flex items-center justify-center" style={{ height:180, background:'#f5f6f8' }}>
        <img src={m.img} alt={m.nom} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500" style={{ padding:'16px' }} onError={e => { (e.currentTarget as HTMLImageElement).style.opacity='0' }}/>
        <span className="absolute top-3 left-3 text-white text-[10px] font-bold px-2.5 py-1 rounded-full" style={{ background:couleur }}>{m.tag}</span>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex gap-1.5 flex-wrap mb-4">
          {m.specs.map((s: string, j: number) => <span key={j} className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background:couleur+'12', color:couleur }}>{s}</span>)}
        </div>
        <div className="flex items-center justify-between pt-3" style={{ borderTop:'1px solid rgba(0,0,0,0.06)' }}>
          ...
          <div className="flex items-center gap-1 text-xs font-semibold group-hover:gap-2 transition-all" style={{ color:couleur }}>
            Voir <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
          </div>
        </div>
      </div>
    </Link>
...
function SerieSection({ serie }: { serie: any }) {
  ...
  return (
    <div ref={ref} className="mb-16">
      <div className="flex items-center justify-between mb-8" style={{ opacity:inView?1:0, transform:inView?'translateX(0)':'translateX(-24px)', transition:'all 0.6s cubic-bezier(0.22,1,0.36,1)' }}>
        ...
        <Link href="/catalogue" className="text-xs font-bold flex items-center gap-1 hover:gap-2 transition-all" style={{ color:serie.couleur }}>
          Voir tout <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
        </Link>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {serie.modeles.map((m: any, i: number) => <ModelCard key={i} m={m} couleur={serie.couleur} index={i}/>)}
      </div>
    </div>
  )
}
    ...
    <section id={marque.id} className="relative" style={{ background:'#fff' }}>
      ...
      <div className="relative max-w-7xl mx-auto px-6 pt-20 pb-12" style={{ zIndex:1 }}>
        <div ref={ref} style={{ opacity:inView?1:0, transform:inView?'translateY(0)':'translateY(24px)', transition:'all 0.7s cubic-bezier(0.22,1,0.36,1)' }}>
          <span className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-white px-4 py-1.5 rounded-full mb-5" style={{ background:marque.couleur }}>
            <span className="w-1.5 h-1.5 rounded-full bg-white/80 animate-pulse"/> Gamme {marque.nom}
          </span>
          <h2 className="font-black leading-none tracking-tighter mb-3" style={{ fontSize:'clamp(2.5rem,6vw,5rem)', color:marque.couleur }}>{marque.nom}</h2>
      ...
      <div className="relative max-w-7xl mx-auto px-6 pb-16" style={{ zIndex:1 }}>
        {marque.series.map(serie => <SerieSection key={serie.id} serie={serie}/>)}
      </div>
...
export default function HomePage() {
  ...
  useEffect(() => {
    setProgress(0)
    let elapsed = 0; const duration = 5000
  ...
  return (
    <main className="bg-white">
        ...
        {heroSlides.map((s,i) => (
          <div key={i} className="absolute inset-0 transition-all duration-700"
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
              <div className="absolute rounded-full" style={{ width:600, height:600, right:-100, top:-150, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.06)' }}/>
              <div className="absolute rounded-full" style={{ width:400, height:400, right:80, top:-50, background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.05)' }}/>
              <div className="absolute rounded-full" style={{ width:200, height:200, right:220, top:80, background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.08)' }}/>
              {[...Array(6)].map((_,j) => <div key={j} className="absolute" style={{ width:1, height:'120%', right:`${15+j*8}%`, top:'-10%', background:'linear-gradient(to bottom, transparent, rgba(255,255,255,0.06), transparent)', transform:'rotate(15deg)' }}/>)}
            </div>
          </div>
        ))}
        ...
        <div className="relative z-[3] h-full flex flex-col justify-center pt-20">
          <div className="max-w-7xl mx-auto px-6 w-full">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-5 text-white text-xs font-black uppercase tracking-widest"
              ...
              <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse"/> CADOZAT · {hero.tag}
            </div>
            ...
            <div className="flex gap-3 flex-wrap" style={{ opacity:transitioning?0:1, transition:'all 0.45s 0.1s' }}>
      ...
      <section style={{ background:'#0f172a', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
        <div className="max-w-7xl mx-auto px-6 py-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 divide-x divide-white/10">
            {[
              { v:'3',    l:'Agences au Maroc',    color:'#4a9eff' },
              { v:'25+',  l:'Modèles disponibles', color:'#ff6b6b' },
              { v:'3 ans',l:'Garantie Isuzu',      color:'#4ade80' },
              { v:'6j/7', l:'Service après-vente', color:'#fbbf24' },
            ].map((s,i) => (
      ...
      <section style={{ padding:'80px 0', background:'#0f172a', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', top:'-100px', right:'-100px', width:'500px', height:'500px', borderRadius:'50%', background:'radial-gradient(circle, rgba(204,0,0,0.15) 0%, transparent 60%)' }} />
        ...
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:'300px', height:'300px', borderRadius:'50%', background:'radial-gradient(circle, rgba(201,168,76,0.08) 0%, transparent 60%)' }} />
        {/* Top gradient line */}
        ...
        <div className="relative max-w-2xl mx-auto px-6 text-center">
          <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'99px', padding:'6px 16px', marginBottom:'24px' }}>
            ...
            <span style={{ fontSize:'11px', color:'rgba(255,255,255,0.5)', fontWeight:700, letterSpacing:'0.15em', textTransform:'uppercase' }}>Votre partenaire mobilité au Maroc</span>
          </div>

          <h2 style={{ fontSize:'clamp(28px,5vw,48px)', fontWeight:900, color:'#fff', letterSpacing:'-1.5px', margin:'0 0 12px', lineHeight:1.05 }}>
            Trouvez votre<br/>
            <span style={{ background:'linear-gradient(90deg, #CC0000, #0057A8)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>véhicule idéal</span>
          </h2>

          <div style={{ width:'40px', height:'3px', background:'linear-gradient(90deg, #CC0000, #0057A8)', borderRadius:'2px', margin:'16px auto' }} />

          <p style={{ color:'rgba(255,255,255,0.4)', fontSize:'14px', marginBottom:'36px', lineHeight:1.7 }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:'12px', justifyContent:'center' }}>
            ...
            <Link href="/devis"
              style={{ display:'inline-flex', alignItems:'center', gap:'8px', background:'rgba(0,87,168,0.3)', color:'rgba(255,255,255,0.85)', fontWeight:700, fontSize:'14px', padding:'14px 28px', borderRadius:'14px', border:'1px solid rgba(0,87,168,0.5)', textDecoration:'none', transition:'all 0.3s ease' }}
              onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background='rgba(0,87,168,0.5)' }}
              onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background='rgba(0,87,168,0.3)' }}>
              Demande de devis
            </Link>
          </div>
        </div>
